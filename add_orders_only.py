#!/usr/bin/env python3
"""
Simple script to add orders only (assumes users, warehouses, and drivers already exist)
"""
...
def add_orders():
    """Add random orders to existing warehouses"""
    app = create_app()
    ...
    with app.app_context():
        ...
        print(f"\n📊 Total orders in database: {Order.query.count()}")
        
        # Show orders by date
        future_orders = Order.query.filter(Order.date >= date(2025, 8, 22)).count()
        present_orders = Order.query.filter(
            Order.date < date(2025, 8, 22),
            Order.date >= date.today() - timedelta(days=7)
        ).count()
        completed_orders = Order.query.filter(
            Order.date < date.today() - timedelta(days=7)
        ).count()
        
        print(f"📈 Future orders (22-23 Aug 2025+): {future_orders}")
        print(f"📅 Present orders (recent): {present_orders}")
        print(f"✅ Completed orders (past): {completed_orders}")
